<?php
	...
	if (isset($_POST['submit'])) {		
		$sql = $conn->prepare("UPDATE barang SET nama=? , size=?, bahan=?, harga=?, kategori=?, keterangan=?  WHERE id=?");
		$nama=$_POST['nama'];
		$size = $_POST['size'];
		$bahan= $_POST['bahan'];
		$harga= $_POST['harga'];
		...
		$sql->bind_param("ssssssi",$nama, $size, $bahan, $harga, $kategori, $keterangan, $_GET["id"]);	
		if($sql->execute()) {
...
<table border="0" cellpadding="10" cellspacing="0" width="500" align="center" class="tbl-qa">
	...
	<tbody>
		...
		<tr class="table-row">
			...
			<td><input type="text" name="kategori" class="txtField" value="<?php echo $row["kategori"]?>"></td>
		</tr>
		<tr class="table-row">
			<td><label>Keterangan</label></td>
			<td><textarea name="keterangan" style="
    width: 250px;
    height: 150px;
    padding: 12px 20px;
    ...
    border-radius: 4px;
    resize: none;
"><?php echo $row["keterangan"]?></textarea></td>
		</tr>
		<tr class="table-row">
