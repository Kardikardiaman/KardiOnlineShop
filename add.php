...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
    	require_once("db.php");
		$sql = $conn->prepare("INSERT INTO barang (nama,size,bahan,harga,kategori,keterangan,gambar) VALUES (?, ?, ?, ?, ?, ?, ?)");  
		$nama=$_POST['nama'];
		$size = $_POST['size'];
		$bahan= $_POST['bahan'];
		$harga= $_POST['harga'];
		...
		$sql->bind_param("sssssss", $nama, $size, $bahan, $harga, $kategori, $keterangan, $gambar); 
		if($sql->execute()) {
...
<table border="0" cellpadding="10" cellspacing="0" width="500" align="center" class="tbl-qa">
	<thead>
		<tr>
			<th colspan="2" class="table-header">Tambahkan Barang Kardi</th>
		</tr>
	</thead>
	<tbody>
		<tr class="table-row">
		...
		<tr class="table-row">
			...
			<td><input type="text" name="kategori" class="txtField"></td>
		</tr>
		<tr class="table-row">
